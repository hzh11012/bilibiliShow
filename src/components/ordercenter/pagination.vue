<template>
  <div class="pagination-box" v-if="$store.state.orderList.total>0">
    <div
      class="arrow left-arrow"
      :class="$store.state.order_page==1?'disabled':''"
      @click="prePage"
    ></div>
    <div class="pagination">
      <span class="pageNum" @click="clickPage(1)" :class="$store.state.order_page==1?'active':''">1</span>
      <span class="pageNum" @click="prePageMore" v-show="showPreMore">...</span>
      <span
        class="pageNum"
        :class="$store.state.order_page==index?'active':''"
        v-for="index in pages"
        :key="index"
        @click="clickPage(index)"
      >{{index}}</span>
      <span class="pageNum" @click="nextPageMore" v-show="showNextMore">...</span>
      <span
        v-if="Math.ceil($store.state.orderList.total/10)>1"
        @click="clickPage(Math.ceil($store.state.orderList.total/10))"
        class="pageNum"
        :class="$store.state.order_page==Math.ceil($store.state.orderList.total/10)?'active':''"
      >{{Math.ceil($store.state.orderList.total/10)}}</span>
    </div>
    <div
      class="arrow right-arrow"
      :class="$store.state.order_page==Math.ceil($store.state.orderList.total/10)?'disabled':''"
      @click="nextPage"
    ></div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      showPreMore: false, //是否显示左省略号
      showNextMore: false //是否显示右省略号
    };
  },
  methods: {
    async getList() {
      this.$emit("getProjects");
      this.toMove();
    },
    prePage() {
      this.$store.commit("order_page", this.$store.state.order_page - 1);
      this.getList();
    },
    prePageMore() {
      this.$store.commit("order_page", this.$store.state.order_page - 3);
      this.getList();
    },
    nextPage() {
      this.$store.commit("order_page", this.$store.state.order_page + 1);
      this.getList();
    },
    nextPageMore() {
      this.$store.commit("order_page", this.$store.state.order_page + 3);
      this.getList();
    },
    clickPage(num) {
      this.$store.commit("order_page", num);
      this.getList();
    },
    toMove() {
        let el = document.getElementsByClassName("top")[0];
        this.$nextTick(function() {
          window.scrollTo({ behavior: "smooth", top: el.offsetTop });
        });
    }
  },
  computed: {
    pages() {
      const foldPage = this.$store.state.foldPage; //规定最多显示的页数tag
      const current = Number(this.$store.state.order_page); //当前页数
      const halfFoldPage = Math.floor((foldPage - 2) / 2);

      if (Math.ceil(this.$store.state.orderList.total / 10) > foldPage) {
        if (current - halfFoldPage > 2) {
          this.showPreMore = true;
        } else {
          this.showPreMore = false;
        }

        if (
          current + halfFoldPage <
          Math.ceil(this.$store.state.orderList.total / 10) - 1
        ) {
          this.showNextMore = true;
        } else {
          this.showNextMore = false;
        }
      }

      let arr = [];

      if (Math.ceil(this.$store.state.orderList.total / 10) - 1 < 5) {
        this.showPreMore = false;
        this.showNextMore = false;
        for (
          let i = 2;
          i < Math.ceil(this.$store.state.orderList.total / 10);
          i++
        ) {
          arr.push(i);
        }
      } else {
        // 左侧省略号显示
        if (this.showNextMore && !this.showPreMore) {
          for (let i = 2; i < foldPage; i++) {
            arr.push(i);
          }
          // 左侧省略号 和 右侧省略号都显示
        } else if (this.showPreMore && this.showNextMore) {
          for (
            let i = current - halfFoldPage;
            i <= current + halfFoldPage;
            i++
          ) {
            arr.push(i);
          }
          // 右侧省略号
        } else if (!this.showNextMore && this.showPreMore) {
          // 当右侧省略号显示的时候,页码不能大于$last,需要往前多显示差额
          let dis =
            current +
            halfFoldPage -
            Math.ceil(this.$store.state.orderList.total / 10) +
            1;

          for (
            let i = current - halfFoldPage - dis;
            i < Math.ceil(this.$store.state.orderList.total / 10);
            i++
          ) {
            arr.push(i);
          }
          // 都不显示
        } else {
          for (
            let i = 2;
            i < Math.ceil(this.$store.state.orderList.total / 10);
            i++
          ) {
            arr.push(i);
          }
        }
      }

      return arr;
    }
  }
};
</script>

<style lang="less" scoped>
.pagination-box {
  margin-bottom: 55px;
  justify-content: center;
  align-items: center;
  display: flex;
  .arrow {
    width: 17px;
    height: 17px;
    background-size: 100% 100%;
    background-repeat: no-repeat;
    cursor: pointer;
  }
  .left-arrow {
    background-image: url(data:image/png;base64,);
  }
  .left-arrow.disabled {
    cursor: auto;
    pointer-events: none;
    background-image: url(data:image/png;base64,);
  }
  .right-arrow {
    background-image: url(data:image/png;base64,);
  }
  .right-arrow.disabled {
    cursor: auto;
    pointer-events: none;
    background-image: url(data:image/png;base64,);
  }
  .pagination {
    padding: 0 30px;
    display: flex;
    .pageNum {
      cursor: pointer;
      z-index: 2;
      display: inline-block;
      font-size: 12px;
      color: #222;
      letter-spacing: 0;
      font-family: Arial-Black;
      padding: 0 13px;
    }
    .active {
      color: #f25d8e;
      position: relative;
      pointer-events: none;
    }
  }
}
</style>